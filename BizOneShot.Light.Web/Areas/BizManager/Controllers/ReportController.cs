...
namespace BizOneShot.Light.Web.Areas.BizManager.Controllers
{
    ...
    public class ReportController : BaseController
    {
        ...
        public async Task<ActionResult> BasicSurveyReport(string curPage)
        {
            ...
            SqlParameter loginId = new SqlParameter("LOGIN_ID", Session[Global.LoginID]);
            object[] parameters = new object[] { loginId };

            var compInfo = await procMngService.baGetReportList(parameters);

            var selectCompInfo = Mapper.Map<List<BasicSurveyReportViewModel>>(compInfo);
            int pagingSize = int.Parse(ConfigurationManager.AppSettings["PagingSize"]);

            return View(new StaticPagedList<BasicSurveyReportViewModel>(selectCompInfo.ToPagedList(int.Parse(curPage ?? "1"), pagingSize), int.Parse(curPage ?? "1"), pagingSize, selectCompInfo.Count));
        }
        ...
        public async Task<ActionResult> BasicSurveyReport(BasicSurveyReportViewModel paramModel, string curPage)
        {
            ...
            //var compMappings = await _scCompMappingService.GetPagedListCompMappingsForBasicReportAsync(int.Parse(curPage), pagingSize, int.Parse(compSn), executorId, paramModel.BizWorkSn);
            //var compInfo = compMappings.Select(cmp => cmp.ScCompInfo).ToList();
            //ViewBag.SelectCopmpInfoList = ReportHelper.MakeCompanyList(compInfo);
            ...
            //return View(new StaticPagedList<BasicSurveyReportViewModel>(rptMasterListView, int.Parse(curPage), pagingSize, compMappings.TotalItemCount));
            #endregion
        ...
        public ActionResult CurrentQueState(string curPage)
        {
        ...
        //    int pageSize = 100;
        //    // scCompMapping 18개 
        //    var scCompMapping = await _scCompMappingService.GetPagedListCompMappingsAsync(int.Parse(curPage), pagingSize, int.Parse(compSn), executorId, paramModel.BizWorkSn);
        //    var compInfo = scCompMapping.Select(cmp => cmp.ScCompInfo).ToList();
        //    ViewBag.SelectCompInfoList = ReportHelper.MakeCompanyList(compInfo);
        //    ViewBag.SelectStatusList = ReportHelper.MakeReportStatusList();
        //    // 18개
        //    var listScCompInfoMapping = await _scCompMappingService.GetCompMappingInfoPagedListByMentorIdForMng(paramModel.BizWorkSn, "A", paramModel.BizWorkYear, int.Parse(curPage), pageSize);

        //    //뷰모델 맵핑
        //    var comListViews = Mapper.Map<List<BasicSurveyReportViewModel>>(listScCompInfoMapping);
        ...
        //    return View(new StaticPagedList<BasicSurveyReportViewModel>(comListViews, int.Parse(curPage), pagingSize, scCompMapping.TotalItemCount));
        ...
        //    var scCompMapping = await _scCompMappingService.GetPagedListCompMappingsAsync(curpage, pagingSize, int.Parse(compSn), executorId, BizWorkSn);
        //    var compInfo = scCompMapping.Select(cmp => cmp.ScCompInfo).ToList();
        //    ViewBag.SelectCompInfoList = ReportHelper.MakeCompanyList(compInfo);

        //    // Mentor와 mapping 된 전체 comp count
        //    var bizList = ReportHelper.MakeCompanyList(compInfo);
        ...
        public async Task<ActionResult> ReportCover(BasicSurveyReportViewModel paramModel)
        {
            ...
            paramModel.CompSn = compInfo[0].COMP_SN;
            paramModel.CompNm = compInfo[0].COMP_NM;
            paramModel.BizWorkYear = compInfo[0].BASIC_YEAR;
            paramModel.QuestionSn = compInfo[0].QUESTION_SN;
            paramModel.Status = compInfo[0].STATUS;
            paramModel.RegistrationNo = compInfo[0].REGISTRATION_NO;
            paramModel.NumSn = compInfo[0].NUM_SN;

            //ViewBag.SelectReportYearList = ReportHelper.MakeBasicSurveyYear(compInfo, paramModel.BizWorkYear);
        ...
        public async Task<ActionResult> DeepenReportList(SelectedMentorTotalReportParmModel param, string curPage)
        {
            ...
            object[] parameters = new object[] { loginId };
            var compList = await procMngService.getBaDeepenReport(parameters);
            ...
            //var compSnList = await procMngService.getDeepenReport(parameters2);
            //var mentoringList = compSnList.Where(bw => bw.CLASSIFY == "A");

            var listTotalReportView =
               Mapper.Map<List<MentoringTotalReportViewModel>>(compList);

            //실제 쿼리
            int pagingSize = int.Parse(ConfigurationManager.AppSettings["PagingSize"]);

            return View(new StaticPagedList<MentoringTotalReportViewModel>(listTotalReportView, int.Parse(curPage ?? "1"), pagingSize, listTotalReportView.Count));
        ...
        public async Task<ActionResult> DeepenReportList(string curPage)
        {
            ...
            return View(new StaticPagedList<MentoringTotalReportViewModel>(listTotalReportView.ToPagedList(int.Parse(curPage), pagingSize), int.Parse(curPage), pagingSize, listTotalReportView.Count));
